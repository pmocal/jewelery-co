...
exports.watch_update_get = function(req, res, next) {
	Watch.findById(req.params.id)
		.exec(function(err, watch) {
			if (err) {
				return next(err);
			...
			res.render('watch_form', { title: 'Update Watch', watch: watch });
		})
}
...
exports.watch_update_post = [
	...
	(req, res, next) => {
		...
		if (!errors.isEmpty()) {
			...
			// Data from form is valid. Save watch.
			Watch.findByIdAndUpdate(req.params.id, watch, {}, function (err, thewatch) {
				if (err) { return next(err); }
					//successful - redirect to new watch record.
